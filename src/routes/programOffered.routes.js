...
import fs from "fs";
import ResearchInFocus from "../models/ProgramOffered.js";
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    const name = `program-${Date.now()}-${Math.round(
      Math.random() * 1e9
    )}${ext}`;
    cb(null, name);
  },
...
router.post("/", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    const research = await ResearchInFocus.create({
      title,
      description,
      image: `uploads/program-offered/${req.file.filename}`,
    });
...
router.put("/:id", upload.single("image"), async (req, res) => {
  try {
    ...
    if (req.file) {
      research.image = `uploads/program-offered/${req.file.filename}`;
    }
